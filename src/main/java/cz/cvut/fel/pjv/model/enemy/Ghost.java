...
import cz.cvut.fel.pjv.model.object.Potion_Red;
...
public class Ghost extends Entity {
    ...
    private void setupGhostImage() {
        try {
            up1 = setup("/enemy/ghost_down_1", gp.tileSize, gp.tileSize);
            up2 = setup("/enemy/ghost_down_2", gp.tileSize, gp.tileSize);
            down1 = setup("/enemy/ghost_down_1", gp.tileSize, gp.tileSize);
            down2 = setup("/enemy/ghost_down_2", gp.tileSize, gp.tileSize);
            left1 = setup("/enemy/ghost_down_1", gp.tileSize, gp.tileSize);
            left2 = setup("/enemy/ghost_down_2", gp.tileSize, gp.tileSize);
            right1 = setup("/enemy/ghost_down_1", gp.tileSize, gp.tileSize);
            right2 = setup("/enemy/ghost_down_2", gp.tileSize, gp.tileSize);
        } catch (Exception e) {
    ...
    public void setAction () {
        ...
        if (!onPath && tileDistance < Tank.CLOSE_DISTANCE) {
            handleRandomPathSelection();
        }

        if (onPath && tileDistance > Tank.FAR_DISTANCE) {
            handleRandomPathSelection();
        }
    ...
    private void handleRandomPathSelection() {
        ...
        if (i > Tank.MEDIUM_LOW_PROBABILITY) {
            onPath = true;
        }
    }
    ...
    private void handleRandomDirection() {
        ...
        if (actionLockCounter == 120) {
            ...
            if (i <= Tank.LOW_PROBABILITY) {
                direction = "up";
            }
            if (i > Tank.LOW_PROBABILITY && i <= Tank.MEDIUM_LOW_PROBABILITY) {
                direction = "down";
            }
            if (i > Tank.MEDIUM_LOW_PROBABILITY && i <= Tank.MEDIUM_HIGH_PROBABILITY) {
                direction = "left";
            }
            if (i > Tank.MEDIUM_HIGH_PROBABILITY) {
                direction = "right";
            }
    ...
    public void checkDrop() {
        ...
        // SET THE ENEMY DROP
        if (i < Tank.MEDIUM_LOW_PROBABILITY) { dropItem(new Potion_Red(gp)); }

        if (i >= Tank.MEDIUM_LOW_PROBABILITY && i < Tank.MEDIUM_HIGH_PROBABILITY) { dropItem(new Heart(gp)); }

        if (i >= Tank.MEDIUM_HIGH_PROBABILITY && i < Tank.HIGH_PROBABILITY) { dropItem(new ManaCrystal(gp)); }
