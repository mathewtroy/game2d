...
public class Player extends Entity {
    ...
    public int getCurrentHelmetSlot() {
        ...
        for (int i = 0; i < inventory.size(); i++) {
            if(inventory.get(i) == currentHelmet) { currentHelmetSlot = i; }
        }
    ...
    private void handleMovementAndInteractions() {
        if (keyH.upPressed) { direction = MapConstants.UP; }
        else if (keyH.downPressed) { direction = MapConstants.DOWN; }
        else if (keyH.leftPressed) { direction = MapConstants.LEFT; }
        else if (keyH.rightPressed) { direction = MapConstants.RIGHT; }
        collisionOn = false;
        ...
        if (!collisionOn && !keyH.enterPressed) { handlePlayerMovement(); }
        if (keyH.enterPressed && !attackCanceled) { startAttack(); }
    ...
    private void handleProjectile() {
        ...
        if (shotAvailableCounter < SHOT_AVAILABLE_COUNTER_THRESHOLD) { shotAvailableCounter++; }
    }
    ...
    public void attacking() {
        ...
        if (spriteCounter > 5 && spriteCounter <= ATTACK_DURATION) {
            ...
            switch(direction) {
                case MapConstants.UP: worldY -= attackArea.height; break;
                case MapConstants.DOWN: worldY += attackArea.height; break;
                case MapConstants.LEFT: worldX -= attackArea.height; break;
                case MapConstants.RIGHT: worldX += attackArea.height; break;
            }
    ...
    public void damageEnemy(int i, int attack, int knockBackPower) {
        if (i != GameConstants.MAX_COST) {
            if (!gp.enemy[gp.currentMap][i].invisible) {
                gp.playSE(GameConstants.SOUND_FIVE);
                if (knockBackPower > 0) { knockBack(gp.enemy[gp.currentMap][i], knockBackPower); }
    ...
    public void draw (Graphics2D g2) {
        ...
        if (invisible) { g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,0.3f )); }
